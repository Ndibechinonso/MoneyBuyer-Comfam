    ...
    <>
      <header className="confam__layout--header">
        <div className="confam__layout--header_top header">
          <NotificationIcon />
          <UserMenu dropDownHandler={dropDownClickHandler} showDropDown={showDropDown} />
          {showDropDown && <DropDownMenu dropDownHandler={dropDownClickHandler} />}
        </div>
        <div className="confam__layout--header_bottom titleBar">
          <div className="titleBar__message">
            <div className="titleBar__message--headline">{`${value?.title} ${ pathname === "/dashboard" ? "Kenneth!":""}`}</div>
            <div className="titleBar__message--sub">{`${value?.subtitle}`}</div>
          </div>
          <CustomButton className="titleBar__cta"  action={() => console.log("first")} actionText="New Transaction" />
        </div>
      </header>
    </>
  )
